...
public class ConstructNetherPortalBucketTask extends Task {
    ...
    protected Task onTick() {
        ...
        if (portalOrigin != null) {
            if (mod.getWorld().getBlockState(portalOrigin.above()).getBlock() == Blocks.NETHER_PORTAL) {
                setDebugState("Done constructing nether portal.");
                mod.getBlockScanner().addBlock(Blocks.NETHER_PORTAL, portalOrigin.above());
                return null;
            }
        }
        ...
        //If too far, reset.
        if (portalOrigin != null && !portalOrigin.closerToCenterThan(mod.getPlayer().position(), 2000)) {
            portalOrigin = null;
            currentDestroyTarget = null;
        }
        ...
        return new InteractWithBlockTask(new ItemTarget(new Item[]{Items.FLINT_AND_STEEL, Items.FIRE_CHARGE}, 1), Direction.UP, portalOrigin.below(), true);
    }
