...
public class GetCloseToBlockTask extends Task {
    ...
    private int getCurrentDistance() {
        return (int) Math.sqrt(AltoClef.getInstance().getPlayer().blockPosition().distSqr(_toApproach));
    }

    private boolean inRange() {
        return AltoClef.getInstance().getPlayer().blockPosition().distSqr(_toApproach) <= _currentRange * _currentRange;
    }
