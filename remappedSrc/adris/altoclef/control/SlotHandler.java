...
public class SlotHandler {
    ...
    private void clickWindowSlot(int windowSlot, int mouseButton, ClickType type) {
        ...
        try {
            mod.getController().handleInventoryMouseClick(syncId, windowSlot, mouseButton, type, player);
        } catch (Exception e) {
    ...
    public void forceEquipItemToOffhand(Item toEquip) {
        ...
        for (Slot CurrentItemSlot : currentItemSlot) {
            if (!Slot.isCursor(CurrentItemSlot)) {
                mod.getSlotHandler().clickSlot(CurrentItemSlot, 0, ClickType.PICKUP);
            } else {
                mod.getSlotHandler().clickSlot(PlayerSlot.OFFHAND_SLOT, 0, ClickType.PICKUP);
            }
        }
    }
